defmodule Chord.Node do
  ...
  def start_link(arg) do
    name = {:via, Registry, {Chord.Registry, "node_#{Enum.at(arg, 1)}"}}
  ...
  def init(arg) do
    ...
    params = Map.merge(arg, %{self: %Peer{name: "node_#{arg.id}", hash: myHash},predecessor: nil,successor: nil,finger: finger,fail_node: fail_node,total_hops: 0,message_sent: 0,alive: not fail_node})
    {:ok, params}
  end

  def handle_cast(:terminate, params) do
    params_new = Map.put(params, :alive, false)
    {:stop, :normal, params_new}
  end
  def handle_cast(:start, params) do
    initial_node = GenServer.call(Chord.Manager, :initial_insert)
    params_new = if initial_node == nil, do: create(params), else: join(params, initial_node)
    :ok = GenServer.call(Chord.Manager, {:create_ring, params_new.self})
    {:noreply, params_new}
  end
  ...
  def handle_call({:params_new, key, value}, _from, params), do: {:reply, :ok, Map.put(params, key, value)}
  ...
  def handle_info(:message, params) do
    cond do
      ...
      params.fail_node ->
          :ok = GenServer.call(Chord.Manager, {:node_completed, params.self.name, 0})
    end
    {:noreply, params}
  end
  ...
  defp find_successor(params, id) do
    if ring_traversal?(id, params.self.hash, false, params.successor.hash, true) do
      ...
      if cl_node == params.self do
        ...
        GenServer.call({:via, Registry, {Chord.Registry, cl_node.name}}, {:find_successor, id})
      end
    end
  end
  ...
  defp find_successor(params, id, callee, hops) do
    if ring_traversal?(id, params.self.hash, false, params.successor.hash, true) do
      ...
      cl_node = closest_preceding_node(params, id, 40 - 1)
      if cl_node == params.self do
        GenServer.reply(callee, {params.self, hops})
      else
        GenServer.cast({:via, Registry, {Chord.Registry, cl_node.name}}, {:find_successor, id, callee, hops})
      end
    end
  end
  ...
  defp join(params, initial_node) do
    {successor, _} = GenServer.call({:via, Registry, {Chord.Registry, initial_node.name}}, {:find_successor, params.self.hash})
    Map.merge(params, %{predecessor: nil, successor: successor})
  end

  defp stabilize(params) do
    x = GenServer.call({:via, Registry, {Chord.Registry, params.successor.name}}, :predecessor)

    updated_successor =
      if x != nil and ring_traversal?(x.hash, params.self.hash, false, params.successor.hash, false) do
        :ok = GenServer.call({:via, Registry, {Chord.Registry, params.self.name}}, {:params_new, :successor, x})
        x
      else
        params.successor
      end

    :ok = GenServer.call({:via, Registry, {Chord.Registry, updated_successor.name}}, {:notify, params.self})

    Process.send_after(Registry.lookup(Chord.Registry, params.self.name) |> Enum.at(0) |> elem(0), :stabilize, 50)
  end
  ...
  defp fix_fingers(params, next) do
    ...
    :ok = GenServer.call({:via, Registry, {Chord.Registry, params.self.name}}, {:params_new, :finger, updated_finger})

    next = if next + 1 >= 40, do: 0, else: next + 1
    Process.send_after(Registry.lookup(Chord.Registry, params.self.name) |> Enum.at(0) |> elem(0), {:fix_fingers, next}, 20)
  end

  defp check_predecessor(params) do
    if params.predecessor != nil and GenServer.whereis({:via, Registry, {Chord.Registry, params.predecessor.name}}) == nil do
      :ok = GenServer.call({:via, Registry, {Chord.Registry, params.self.name}}, {:params_new, :predecessor, nil})
    end
    Process.send_after(Registry.lookup(Chord.Registry, params.self.name) |> Enum.at(0) |> elem(0), :check_predecessor, 50)
  end

  defp send_message(params, message_hash) do
    {_result, hops} = find_successor(params, message_hash)
    :ok = GenServer.call({:via, Registry, {Chord.Registry, params.self.name}}, {:params_new, :total_hops, params.total_hops + hops})
    :ok = GenServer.call({:via, Registry, {Chord.Registry, params.self.name}}, {:params_new, :message_sent, params.message_sent + 1})
  end
